...
9. **Shared semantics defined once** — When several dispatch steps depend on the same rule (strict mode, abort-on-failure, pagination termination, retry budget), the plan must define it in one shared section that each step references; a rule restated per step drifts (one step strict, another lenient). Any configuration key, annotation, or default value the plan tells an agent to write must cite the file it was read from — a default supplied from memory silently changes fail-loud into fail-silent. Both are [Major] issues.
